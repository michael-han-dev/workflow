...
import { getWorld, getWorldHandlers, initWorld } from './world.js';
...
const lazyStepHandler = async (req: Request): Promise<Response> => {
  if (!initPromise) {
...
export const stepEntrypoint: (req: Request) => Promise<Response> =
  /* @__PURE__ */ withHealthCheck(lazyStepHandler);
