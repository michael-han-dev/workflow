...
This guide will walk through setting up your first workflow in a Tanstack Start app. Along the way, you'll learn more about the concepts that are fundamental to using the development kit in your own projects.
...
Start by creating a new Tanstack Start project. This command will create a new directory named `my-workflow-app` with a minimal setup and setup a Tanstack Start project inside it.
...
To invoke your new workflow, we'll have to add your workflow to a `POST` API route handler, `src/routes/api/signup.ts` with the following code:
...
Check the Tanstack Start development server logs to see your workflow execute as well as the steps that are being processed.
