---
title: Astro
description: Set up Workflow DevKit with Astro for durable, long-running functions.
---

<GettingStartedIntro framework="Astro" />
...
Start by creating a new Astro project. This command will create a new directory named `my-workflow-app` and set up a minimal Astro project inside it.
...
```ts title="astro.config.mjs" lineNumbers
// @ts-check
...
```ts title="src/workflows/user-signup.ts" lineNumbers
...
```ts title="src/pages/api/signup.ts" lineNumbers
...
To start your development server, run the following command in your terminal in the Astro root directory:
