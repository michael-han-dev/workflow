---
title: Vite
description: Set up Workflow DevKit with Vite for durable, long-running functions.
---

<GettingStartedIntro framework="Vite" />
...
Start by creating a new Vite project. This command will create a new directory named `my-workflow-app` with a minimal setup and set up a Vite project inside it.
...
```ts title="workflows/user-signup.ts" lineNumbers
...
```ts title="api/signup.post.ts" lineNumbers
