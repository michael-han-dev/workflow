---
title: Nitro
description: Set up Workflow DevKit with Nitro for durable, long-running functions.
---

<GettingStartedIntro framework="Nitro" />
...
```ts title="nitro.config.ts" lineNumbers
...
=======
```ts title="server/api/signup.post.ts" lineNumbers
import { start } from 'workflow/api';
import { defineEventHandler } from 'nitro/h3';
>>>>>>> b82f27c8 (docs: update getting started intro comp)
