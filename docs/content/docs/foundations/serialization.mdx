...
```ts title="workflows/webhook.ts" lineNumbers
...
```ts title="workflows/api-call.ts" lineNumbers
...
```ts title="Implementation" lineNumbers
...
```ts title="workflows/incorrect-mutation.ts" lineNumbers
...
```ts title="workflows/correct-mutation.ts" lineNumbers
export async function updateUserWorkflow(userId: string) {
  "use workflow";
